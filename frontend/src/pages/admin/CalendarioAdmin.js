...
export default function CalendarioAdmin({
    ...
    const [approvazioneMeetingModal, updateApprovazioneMeetingModal] = useState({
        show: false,
        event: null
    })
    const [approvazioneCancellazioneMeetingModal, updateApprovazioneCancellazioneMeetingModal] = useState({
        show: false,
        event: null
    })
        ...
        <>
                                ...
                                <Row key={uuidv4()}>
                                    ...
                                    {weekArray
                                        .map((date) => {
                                            ...
                                            const [occupiedArray] = googleCalendarEvents
                                                .filter(event => {
                                            ...
                                            return (
                                                <Col
                                                    ...
                                                    onClick={() => {
                                                        if(isOccupied){
                                                            if(occupiedArray.description.stato === EVENT_STATE.DA_APPROVARE){
                                                                updateApprovazioneMeetingModal(old =>({
                                                                    show: true,
                                                                    event: occupiedArray
                                                                }))
                                                            } else if(occupiedArray.description.stato === EVENT_STATE.IN_CANCELLAZIONE){
                                                                updateApprovazioneCancellazioneMeetingModal({
                                                                    show: true,
                                                                    event: occupiedArray
                                                                })
                                                            } else if(occupiedArray.description.stato === EVENT_STATE.CONFERMATO){
                                                    ...
                                                    className={`border-2 text-center meetingrooms-timetable-slot ${isOccupied ? 'occupied' : ''} admin ${eventState}`}>
            ...
            <YesNoModal
                ...
                onYes={() => {
                    ...
                    console.log(event)
                    const add0 = x => x<10 ? '0'+x : x
                    const data = add0(event.date.month+1) + add0(event.date.day) + add0(event.date.hour)
                    axios.post('/mqtt/', {
                        sala: event.location,
                        meetingName: event.summary,
                        meetingTime: data,
                        meetingHour: event.date.hour
                    })
